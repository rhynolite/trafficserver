 ...
 *      Author: Brian Geffon <bgeffon@linkedin.com>
 *
 *
...
const std::string DEFAULT_ERROR_RESPONSE =
    "<html><body><h1>Your LinkedIn Network Will Be Back Soon</h1></body></html>";
...
bool BoomResponseRegistry::has_code_registered(int http_status_code) {
  ...
  if (std::find_if(error_codes_.begin(), error_codes_.end(), IsRewritableCode(http_status_code))
      == error_codes_.end())
    return false;
...
public:
  BoomGlobalPlugin() {
    // Register the hook for response header
    registerHook(HOOK_READ_RESPONSE_HEADERS);
  }

  Boom(const BoomResponseRegistry& response_registry) :
...
void TSPluginInit(int argc, const char *argv[]) {
  ...
  BoomGlobalPlugin *pboom = NULL;

  BoomResponseRegistry registry;
  ...
  if (argc == 3) {
    ...
    registry.populate_error_responses(base_directory);
    ...
    registry.register_error_codes(error_codes);

    pboom = new BoomGlobalPlugin(registry);
  } else {
    pboom = new BoomGlobalPlugin();
  }
}
